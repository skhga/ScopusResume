...
export default function SignInPage() {
  ...
  return (
    <div>
      ...
      <button className="w-full flex items-center justify-center gap-2 border border-gray-300 rounded-lg py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 mb-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <Link to="/reset-password" className="text-sm text-brand-600 hover:underline">Forgot password?</Link>
        </div>
        <Button type="submit" loading={loading} className="w-full">Sign In</Button>
      </form>
      <p className="text-center text-sm text-gray-600 mt-6">Don't have an account? <Link to="/signup" className="text-brand-600 font-medium hover:underline">Sign Up</Link></p>
    </div>
  );
}
