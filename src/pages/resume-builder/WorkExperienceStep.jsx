...
import { Plus, Trash2, Sparkles, Check, X, Loader2 } from 'lucide-react';
...
export default function WorkExperienceStep({ data, onChange }) {
  const items = data || [];
  // aiState[`${i}-${j}`] = { loading, suggestion, error }
  ...
  const setAi = (key, patch) => setAiState(prev => ({ ...prev, [key]: { ...prev[key], ...patch } }));
  const clearAi = (key) => setAiState(prev => { const s = { ...prev }; delete s[key]; return s; });

  const rewriteBullet = async (i, j) => {
    const bullet = items[i].bulletPoints[j];
    if (!bullet.trim()) return;
    const key = `${i}-${j}`;
    setAi(key, { loading: true, suggestion: null, error: null });
    try {
      ...
      setAi(key, { loading: false, suggestion: rewritten, error: null });
    } catch {
      setAi(key, { loading: false, suggestion: null, error: 'Could not generate rewrite. Try again.' });
      setTimeout(() => clearAi(key), 3000);
    }
  };

  const acceptSuggestion = (i, j) => {
    const key = `${i}-${j}`;
    updateBullet(i, j, aiState[key]?.suggestion || '');
    clearAi(key);
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Describe what you did in plain language - the AI will optimize it.</p>
      </div>
        ...
        renderItem={(item, i) => (
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2 mb-2">
              <input type="checkbox" checked={item.isCurrentRole || false} onChange={e => update(i, 'isCurrentRole', e.target.checked)} className="rounded border-gray-300 text-brand-600" />
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              {!item.isCurrentRole && <>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">End Month</label><select value={item.endMonth} onChange={e => update(i, 'endMonth', e.target.value)} className="input-field bg-white"><option value="">Month</option>{MONTHS.map(m => <option key={m} value={m}>{m}</option>)}</select></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">End Year</label><input value={item.endYear} onChange={e => update(i, 'endYear', e.target.value)} className="input-field" placeholder="2024" /></div>
              </>}
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">What you did (plain language)</label>
              <textarea value={item.jobDescriptionRaw} onChange={e => update(i, 'jobDescriptionRaw', e.target.value)} rows={3} className="input-field" placeholder="Describe your responsibilities and achievements..." />
            </div>
            <div>
                ...
                return (
                  <div key={j} className="mb-3">
                    <div className="flex items-center gap-2">
                      <span className="text-gray-400 text-sm w-4 shrink-0">{j + 1}.</span>
                      <input
                        value={b}
                      ...
                      <button
                        ...
                        onClick={() => rewriteBullet(i, j)}
                        disabled={ai.loading || !b.trim()}
                        className="p-2 text-brand-600 hover:bg-brand-50 rounded-lg disabled:opacity-40 disabled:cursor-not-allowed"
                      >
                        {ai.loading
                      ...
                      {item.bulletPoints.length > 1 && (
                        <button type="button" onClick={() => removeBullet(i, j)} className="p-2 text-gray-400 hover:text-red-500">
                          <Trash2 className="h-4 w-4" />
                        </button>
                      )}
                    </div>
                    {ai.suggestion && (
                      <div className="mt-2 ml-6 p-3 bg-brand-50 border border-brand-200 rounded-lg">
                        <p className="text-sm text-gray-700 mb-2">{ai.suggestion}</p>
                        <div className="flex gap-2">
                          <button
                            type="button"
                            onClick={() => acceptSuggestion(i, j)}
                            className="flex items-center gap-1 text-xs bg-brand-600 text-white px-3 py-1 rounded-full hover:bg-brand-700"
                          >
                          ...
                          <button
                            type="button"
                            onClick={() => clearAi(key)}
                            className="flex items-center gap-1 text-xs border border-gray-300 text-gray-600 px-3 py-1 rounded-full hover:bg-gray-50"
                          >
                    ...
                    {ai.error && (
                      <p className="mt-1 ml-6 text-xs text-red-500">{ai.error}</p>
                    )}
                  </div>
                );
              })}
              <button type="button" onClick={() => addBullet(i)} className="text-sm text-brand-600 hover:underline flex items-center mt-1">
