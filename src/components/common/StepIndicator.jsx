...
export default function StepIndicator({ steps = [], currentStep = 0, onStepClick }) {
  return (
          ...
          return (
            <li key={i} className={`flex items-center ${i < steps.length - 1 ? 'flex-1' : ''}`}>
              <button
                ...
                className={`flex flex-col items-center ${onStepClick && (done || active) ? 'cursor-pointer' : 'cursor-default'}`}
              >
